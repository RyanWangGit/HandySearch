...
private:
     ...
     */
    float weight;
    Webpage::WeightType type;
    void extractText(const QString &fileContent);
    void extractTitle(const QString &fileContent);
    bool load();
public:
    Webpage();
    Webpage(const QString &filePath);
    Webpage(const Webpage &c);
    Webpage::WeightType getWeightType() const;
    void setWeightType(Webpage::WeightType type);
